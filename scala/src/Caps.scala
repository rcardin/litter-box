...
/** Capability traits (yaes-style: passed as `using` context parameters).
  ...
  * subprocess/fs/gh handlers are slice 2 (Live.scala). Machine.iterate is pure decision
  * logic over these — it touches the world through nothing else.
  *
  * Deviation from the design doc's capability table: `pickIssue` is split into
  * `inProgressIssue` / `oldestReadyIssue` so the resume-in-progress-first decision stays in
  * Machine (under test) instead of hiding inside a handler.
  */
...
/** status.jsonl appender. Pure observability: a wrong event is a wrong banner, never a
  * wrong merge. Sanitization/normalization happens in Machine before the event reaches here.
  */
trait StatusLog:
  def append(event: StatusEvent): Unit

/** Notify seam. Fires on exactly: needs-human terminals, rc-50 exits, successful
  * auto-merges. A dead channel must never change loop behavior (live handler swallows).
  */
trait Notify:
