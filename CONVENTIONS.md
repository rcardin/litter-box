...
- No code in `src/Kit.scala` or `src/KitMacro.scala` may name anything declared outside those two
  files plus `src/Domain.scala` and `src/Caps.scala`. Comments and string literals are exempt.
